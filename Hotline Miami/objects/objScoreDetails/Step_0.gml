...
i=0 repeat (7) {
draw_line_color(0,12+i*3,400,12+i*3,merge_color(c_black,c_maroon,0.25+lengthdir_x(0.25,dir2*4+i*35)),merge_color(c_black,c_maroon,0.25+lengthdir_x(0.25,dir2*4+i*35)+180))
i+=1
}
...
draw_text(387,22,string_hash_to_newline("HIGHSCORE: "+string(highscore)+"#SCORE: "+string(myscore)))
draw_set_color(merge_color(c_yellow,c_orange,0.5+lengthdir_x(0.5,dir2*10)))
draw_text(388,22,string_hash_to_newline("HIGHSCORE: "+string(highscore)+"#SCORE: "+string(myscore)))

draw_set_halign(fa_center)
if gradeon=1 {
draw_set_color(merge_color(c_lime,c_green,0.5+lengthdir_x(0.5,dir2*10)))
draw_text(200,22,string_hash_to_newline("PLAY STYLE:#"+"\""+type+"\""))
draw_set_color(merge_color(c_lime,c_green,0.5+lengthdir_x(0.5,dir2*10+180)))
draw_text(199,22,string_hash_to_newline("PLAY STYLE:#"+"\""+type+"\""))
}
...
draw_text(350-string_width(string_hash_to_newline(mystring))*0.5+lengthdir_x(3,cdir+i*40),190-ii*(12),string_hash_to_newline(mystring))
i-=1
ii+=1
}
...
draw_text_transformed(350-string_width(string_hash_to_newline(mystring))*0.325,210+i*0.1,string_hash_to_newline(mystring),0.65+i*0.005,1+i*0.025,0)
i+=1
}
} else {
if gradex<280 gradex+=10 else {if gradeon=0 {gradeon=1 if grade="A+" SteamSetAchievement("ACH_ACED_IT") scrCheckGrades() alpha=1}}
...
draw_set_halign(fa_left)
draw_text_transformed(290,210,string_hash_to_newline(grade),0.95+amount*0.05,0.9+amount*0.1,0)
}
}
surface_reset_target()
