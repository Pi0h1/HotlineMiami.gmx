...
if scrCollisionLineExt(x,y,objPlayer.lastx,objPlayer.lasty,4,objSolid,objDoorV,objWallSoftH) or object_index=objEnemyFat or object_index=objInspector nothing=1 else {
mask_index=sprMask
global.shake=7
...
SteamIncStat("Dogs",1)
if isAttacking_Blunt {audio_play_sound(sndHit,0,false) audio_play_sound(sndPunch,0,false) my_id.sprite_index=sprDogDeadBlunt my_id.image_index=3+floor(random(3))} else {my_id.sprite_index=sprDogDeadCut audio_play_sound(choose(sndCut1,sndCut2),0,false) my_id.image_index=4+floor(random(3))}
} else {
if isAttacking_Blunt {audio_play_sound(sndHit,0,false) audio_play_sound(sndPunch,0,false) my_id.sprite_index=choose(sprEBackBlunt,sprEBackBlunt,sprEFrontBlunt) my_id.image_index=4+floor(random(11))} 
if scrIsPolice(object_index) and (isAttacking_Blunt) {
...
if isAttacking_Cut {my_id.sprite_index=sprEBackCut audio_play_sound(choose(sndCut1,sndCut2),0,false) my_id.image_index=2+floor(random(3)) if my_id.image_index=3 or my_id.image_index=4 scrGoreCut(x,y,edir-180,1) else scrGoreCut(x,y,edir-180,0)}
if isAttacking_Slash {
