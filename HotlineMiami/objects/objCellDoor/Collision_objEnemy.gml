if (solid)
    exit;
if (other.object_index == objDogPatrol || other.object_index == objEnemyFat) {
    swinger = 2;
    if (abs(swingspeed) < 2)
    ...
    if (swinger == 1 || swinger == 0) {
        ...
        with (other) {
            if (object_get_parent(object_index) == objEnemyIdle) {
                if (room == rmTrainstationEntrance || room == rmHouse3Downstairs)
                    sprite_index = sprEWalkKnife;
                else sprite_index = choose(sprEWalkClub, sprEWalkBat, sprEWalkKnife, sprEWalkPipe, sprEWalkM16, sprEWalkShotgun);
                ammo = 0;
