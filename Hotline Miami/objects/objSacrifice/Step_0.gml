if instance_exists(objEnemy) or instance_exists(objKnockedOut) or instance_exists(objLevelComplete) exit
...
if wait>0 wait-=1 else {scrGetMessage(1) state=2 wait=20} 
}
}
...
if state=2 {
if !instance_exists(objPhoneConversation) {
if wait>0 wait-=1 else {scrGetMessage(2) state=3 wait=20} 
}
}
...
with objFocus instance_destroy()
