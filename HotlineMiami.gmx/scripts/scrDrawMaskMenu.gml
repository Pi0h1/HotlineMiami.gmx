if (global.selectmask == 0) {
    ...
    with (objMaskMenu) {
        ...
        repeat(masks) {
            if (select == i)
                draw_sprite_ext(sprite_index, i, 81, masky + starty + 22, 1.5, 1.5, lengthdir_x(5, starty * 1.765 + dir * 2), c_black, 0.5);
            else draw_sprite_ext(sprite_index, i, 80 + 1, masky + starty + 2, 1, 1, 0, c_black, 0.5);
            color = c_ltgray;
            if (global.masks[i] == 0)
                color = c_black;
            if (select == i)
                drawy = starty;
            else draw_sprite_ext(sprite_index, i, 80, masky + starty, 1, 1, 0, color, 1);
            if (select == i)
                starty += 80 * 1.5;
            else starty += 80;
            i++;
