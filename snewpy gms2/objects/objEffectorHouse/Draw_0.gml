...
global.mousey=objPlayer.y+((display_mouse_get_y()-height*0.5)*(camera_get_view_width(view_camera[0])/height))*factor
}
x=global.mousex
y=global.mousey
...
draw_set_font(fntScore)
if ammoy>-32 draw_text_transformed(18+string_width(string_hash_to_newline(ammostring))*0.5+lengthdir_x(1,dir),camera_get_view_height(view_camera[0])-ammoy+lengthdir_y(1,dir),string_hash_to_newline(ammostring),1,1,lengthdir_x(2,dir))
draw_set_color(color2)
if ammoy>-32 draw_text_transformed(18+string_width(string_hash_to_newline(ammostring))*0.5+lengthdir_x(1,dir-180),camera_get_view_height(view_camera[0])-ammoy+lengthdir_y(1,dir-180),string_hash_to_newline(ammostring),1,1,lengthdir_x(2,dir-10))
}
}
...
} else {if next=1 {if newsong=1 {sxeasy_stop(1) if room=rmDream1 sxeasy_setVolume(0) sxeasy_play(song);} if room=rmTutorial sxeasy_stop(1) room=nextroom} else {scrRestart()}}}
}
coldir+=1
