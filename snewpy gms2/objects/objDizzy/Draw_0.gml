...
if dizziness>0.1 dizziness-=0.005
if h1>255 h1-=255
if h2>255 h2-=255
if h1<0 h1+=255
if h2<0 h2+=255
draw_set_blend_mode(bm_add)
draw_circle_color(view_get_wport(0)/2,view_get_hport(0)/2,view_get_wport(0)*0.75,c_black,make_color_hsv(0,0,255*(dizziness*(1/2))),0)
...
} else {if next=1 {if newsong=1 {sxeasy_stop(1) if room=rmDream1 sxeasy_setVolume(0) sxeasy_play(song);} room=nextroom} else {scrRestart()}}}
}
coldir+=1
if show_date=1 {
...
screen_refresh()
if round(random(100))=2 myx1=-dizziness*20+random(dizziness*40)
if round(random(100))=2 myx2=-dizziness*20+random(dizziness*40)
if round(random(100))=2 myy1=-dizziness*20+random(dizziness*40)
if round(random(100))=2 myy2=-dizziness*20+random(dizziness*40)
//if check=1 scrSurfaceTiles()
