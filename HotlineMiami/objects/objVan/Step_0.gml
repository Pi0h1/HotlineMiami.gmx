...
if (speed > 0) {
    ...
    if y > 216 {
        if (!brake) {
            brake = 1;
            audio_play_sound(sndCarSkid, 0, false);
            audio_stop_sound(sndCarThrottle);
        }
        friction = 0.38;
        direction += speed * 0.4;
    ...
    if (!solid)
        mp_grid_add_instances(global.grid, objVan, 1);
    solid = 1;
    if (state == 0) {
    ...
    if (state == 5) {
        if (instance_exists(objPlayerMouse)) {
            if (instance_number(objEnemy) + instance_number(objKnockedOut) + instance_number(objBurningGuy) == 0) {
                objEffector.brus = 10;
                with (objBottleExplosion) {
                    my_id = instance_create(x, y, objBombGround) my_id.image_angle = random(360);
                    instance_destroy();
                }
                with (objWeapon)
                    instance_destroy();
