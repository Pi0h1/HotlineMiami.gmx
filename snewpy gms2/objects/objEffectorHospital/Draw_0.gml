...
if objPlayer.object_index=objPlayerMouse or objPlayer.object_index=objPlayerMouseHouse or objPlayer.object_index=objPlayerBiker or objPlayer.object_index=objPlayerBikerHouse or objPlayer.object_index=objPlayerHospital  {
...
} else {if next=1 {if newsong=1 {sxeasy_stop(1) if room=rmDream1 sxeasy_setVolume(0) sxeasy_play(song);} room=nextroom} else {game_load(working_directory+"\\tempsave.sav")}}}
}
coldir+=1
...
draw_set_blend_mode(bm_subtract)
draw_circle_color(camera_get_view_width(view_camera[0])/2,camera_get_view_height(view_camera[0])/2,240+random(24)+24+24,c_black,merge_color(c_navy,c_aqua,0.25+random(0.25)),0)
draw_set_blend_mode(bm_normal)
