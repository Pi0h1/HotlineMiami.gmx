function scrInstanceNearest(argument0, argument1) {
	...
	if (ds_list_size(global.list) > 0)
	    global.result = ds_list_find_value(global.list, argument1);
	else global.result = -12324;
	if (global.result > 0) {
	    with (argument0)
	        if (point_distance(x, y, global.myx, global.myy) == global.result)
	            global.instance = id;
