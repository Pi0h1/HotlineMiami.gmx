...
repeat(6) {
    ...
    if (i < current + 1) {
        ...
        if (amount[i] < 180)
            ...
            draw_text_transformed(room_width - 66, 64 + i * 16, string_hash_to_newline(string(drawbonus[i])), 1, 1, 0);
            draw_set_color(merge_color(c_aqua, c_teal, 0.35 + lengthdir_x(0.35, dir * 24)));
            draw_text_transformed(room_width - 68, 64 + i * 16, string_hash_to_newline(string(drawbonus[i])), 1, 1, 0);
            draw_set_color(c_white);
            draw_text_transformed(room_width - 70, 64 + i * 16, string_hash_to_newline(string(drawbonus[i])), 1, 1, 0);
            draw_set_halign(fa_center);
