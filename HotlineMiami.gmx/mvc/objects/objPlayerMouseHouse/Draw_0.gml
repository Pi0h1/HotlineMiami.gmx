...
if (aimon) {
    ...
    if (global.xbox) {
        if (active) {
            ...
            if (point_distance(x, y, previousx, previousy) > 0.1)
                dir = point_direction(previousx, previousy, x, y);
        }
    }
...
if (sprite_index == sprPThrowMask) {
    ...
    if (image_index >= 3 && image_index < 4)
        draw_sprite_ext(sprMaskDropBack, global.maskindex, x + lengthdir_x(4, dir), y + lengthdir_y(4, dir), 1, 1, dir, image_blend, image_alpha);
    if (image_index >= 4 && image_index < 5)
        draw_sprite_ext(sprMaskDropBack, global.maskindex, x + lengthdir_x(6, dir), y + lengthdir_y(6, dir), 1, 1, dir, image_blend, image_alpha);
    if (image_index >= 5 && image_index < 16)
        draw_sprite_ext(sprMaskDropBack, global.maskindex, x + lengthdir_x(7, dir), y + lengthdir_y(7, dir), 1, 1, dir, image_blend, image_alpha);
}
