if (objEffector.brus == 0) {
    ...
    if sprite_index = sprBoss3Bash {
        image_index = 0;
        image_speed = 0;
        sprite_index = sprBoss3BreakArm;
        objEffector.brus = 10;
        exit;
    }
    if (sprite_index == sprBoss3BreakArm) {
        if (image_index > 8.5 && image_index < 8.8)
            audio_play_sound(sndNeck, 0, false);
            if (image_index < 11)
                image_index += 0.25;
        else {
            objEffector.brus = 10;
            sprite_index = sprBoss3Dead;
        }
        exit;
    }
    if (sprite_index == sprBoss3Dead) {
        if (index < 18)
            ...
            objPlayer.y = y - 18;
            objEffector.brus = 10;
            sprite_index = sprBoss3Burning;
            image_speed = 0.15;
        }
        if (index >= 6 && index - 0.2 < 6)
            audio_play_sound(sndLighter, 0, false);
        if (index >= 10 && index - 0.2 < 10)
            audio_play_sound(sndLighter, 0, false);
        exit;
    }
