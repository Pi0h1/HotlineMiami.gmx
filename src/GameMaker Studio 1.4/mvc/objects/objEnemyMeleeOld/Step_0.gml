...
friction=0
if image_speed<speed*0.1 image_speed=speed*0.1
if !place_free(x+lengthdir_x(speed*8,direction),y+lengthdir_y(speed*8,direction)) direction+=10 else {if frac(direction*(1/90))>0 direction+=10}
} else {
if image_speed<speed*0.1 image_speed=speed*0.1
...
if !instance_exists(objPlayer) {alert=0 direction=round(direction*(1/10))*10 speed=0 exit}
scrSeePlayer()
...
mask_index=sprMask
if point_distance(x,y,objPlayer.x,objPlayer.y)>16 {if speed<1 speed=1 motion_add(point_direction(x,y,objPlayer.x,objPlayer.y),0.1) friction=0} else friction=0.5
