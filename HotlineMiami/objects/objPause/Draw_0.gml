if (sprite_exists(global.sprite)) {
    if (on > 0) {
        ...
        if (!on) {
            ...
            if (blacky > 0)
                ...
                if (sprite_exists(global.sprite))
                    sprite_delete(global.sprite);
                if (file_exists(working_directory + "\\pause.bmp"))
    ...
    if (width < 1200) {
        ...
        draw_sprite_ext(global.sprite, 0, myx, myy, sprx, spry, 0, merge_color(c_white, color1, blur * 0.5), 0.8 + lengthdir_x(0.2, dir));
        draw_sprite_ext(global.sprite, 0, -myx, -myy, sprx, spry, 0, merge_color(c_white, color1, blur * 0.5), 0.8 - lengthdir_x(0.2, dir));
    }
}
