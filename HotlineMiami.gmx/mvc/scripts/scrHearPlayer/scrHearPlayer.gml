...
with (objEnemy) {
    ...
    if (object_index == objEnemyStationary || object_get_parent(object_index) == objEnemyIdle || object_index == objChief)
        valid = 0;
    else {
        if (alert > 0)
            valid = 0;
            ...
            if (point_distance(x, y, objPlayer.x, objPlayer.y) > dist)
                valid = 0;
            else {
...
with (objEnemy) {
    if (path_exists(path) && valid) {
        ds_list_add(global.list, path_get_length(path));
    }
}

ds_list_sort(global.list, 1);
while (ds_list_size(global.list) > 3)
    ds_list_delete(global.list, 3);
