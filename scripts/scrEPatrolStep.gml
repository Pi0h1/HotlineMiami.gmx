...
if alert=0 {
    ...
    if returning=0 {if speed=0 {speed=1*delta direction=startdir} scrPatrol()} else {//if point_distance(x,y,startx,starty)>4 scrReturn(path) else {
...
if !instance_exists(objPlayer) {alert=0 if speed>(1*delta) speed=(1*delta) exit}
