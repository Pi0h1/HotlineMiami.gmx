...
if (sprite_index == sprVanTop || sprite_index == sprVanDoorsOpen) {
    draw_sprite_ext(sprVanDoor, 0, x + doorx, y + doory, 1, 1, image_angle + doorangle, c_white, 1);
    if (doorangle < 90)
        doorangle += 3;
}
if (sprite_index == sprVanDoorsOpen) {
    if (image_index > 20 && image_index < 48) {
        if (instance_exists(objPlayer))
            headdir = scrRotate(headdir, point_direction(x, y, objPlayer.x, objPlayer.y), 6);
