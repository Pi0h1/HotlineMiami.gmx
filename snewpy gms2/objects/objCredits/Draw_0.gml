...
if rollcredits=0 camera_set_view_angle(view_camera[0], lengthdir_x(2,dir*0.2)) else {if topicalpha<=0.05 camera_set_view_angle(view_camera[0], 0)}
d3d_set_projection_ortho(0,0,400,256,camera_get_view_angle(view_camera[0]))
if rollcredits=1 {draw_set_font(fntCreditList) if credit_y<string_height(string_hash_to_newline(creditlist))+128 {if credits_y<1 credits_y+=0.2 else {credits_y=0} if credits_y=1 credit_y+=1}}
...
//if rollcredits=1 and credits_y>0.9 credit_y+=1
d3d_set_projection_ortho(0,0,400,256,0)
...
draw_set_blend_mode(bm_normal)
} else {sxeasy_stop() global.parttitle[0]="PART FIVE." global.parttitle[1]="ANSWERS." global.nextlevel=rmRewind room_goto(rmPart)}
}
