...
if (state == 3) {
    if (!instance_exists(objPhoneConversation)) {
        if (wait > 0)
            ...
            if (objPlayer.image_index > 9 && !shoot) {
                shoot = 1;
                with (objPlayer) {
...
if (state == 4) {
    if (wait > 0) {
        wait--;
        sxeasy_setVolume(wait * (1 / 60));
        if (wait == 0) {
            sxeasy_setVolume(1);
            sxeasy_play(working_directory + "\\ElectricDreams.mp3");
        }
    } else {
        with (objPlayer) {
            lastx = x;
            ...
            if (lastx == x && lasty == y) {
                ...
                image_index += 0.15;
                legindex += 0.25;
                dir = point_direction(lastx, lasty, x, y) legdir = dir;
            }
        }
    }
}
...
if (state == 5) {
    if (wait > 0) {
        ...
        with (objPlayer) {
            ...
            if (!global.throw && image_index > 16) {
                my_id = instance_create(x - 12, y, objMaskDrop);
                my_id.direction = 180 + random(10) my_id.speed = 3;
                my_id.image_angle = 270;
                my_id.sprite_index = sprMaskDropBack;
                global.throw = 1;
            }
        }
    }
...
if (state == 6) {
    if (wait > 0) {
        ...
        with (objPlayer) {
            ...
            if (image_index > 122 && !global.picture) {
                global.picture = 1;
                my_id = instance_create(x - 7, y - 13, objPicture);
                my_id.direction = 90;
                my_id.speed = 1.5;
            }
        }
