...
if (select == 0)
    draw_set_color(merge_color(c_red, c_fuchsia, 0.5 + lengthdir_x(0.5, dir)));
else draw_set_color(merge_color(c_maroon, c_purple, 0.5 + lengthdir_x(0.5, dir)));
draw_text(room_width / 2 - 60 + 2, room_height / 2 + 16 + 2, string_hash_to_newline("YES"));
if (select == 0) {
    draw_set_color(c_white);
    dir1 += 4;
} else {
    draw_set_color(c_ltgray) dir1 = 180;
}
...
if (fade) {
    if (amount > 0) {
        ...
        if (select == 0) {
            ...
            SteamInit();
            global.steamon = 1;
            file = file_text_open_write(working_directory + "\\steamon");
            file_text_close(file);
        } else {
            global.steamon = 0;
            file = file_text_open_write(working_directory + "\\steamoff");
            file_text_close(file);
        }
        room_goto_next();
    }
