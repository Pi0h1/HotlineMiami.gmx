function scrPlayerHospitalMove() {
	if (global.xbox)
	    scrXboxPlayerHospitalMove();
	else {
	    if (sprite_index == sprPHospitalHeadache) {
	        ...
	        if (myxspeed > 2 + lengthdir_x(0.25, global.dir * 3))
	            myxspeed = 2 + lengthdir_x(0.25, global.dir * 3);
	        if (myyspeed > 2 + lengthdir_y(0.25, global.dir * 3))
	            myyspeed = 2 + lengthdir_y(0.25, global.dir * 3);
	        if (myxspeed < -2 + lengthdir_x(0.25, global.dir * 3))
	            myxspeed = -2 + lengthdir_x(0.25, global.dir * 3);
	        if (myyspeed < -2 + lengthdir_y(0.25, global.dir * 3))
	            myyspeed = -2 + lengthdir_y(0.25, global.dir * 3);
	            ...
	            with (objDizzy) {
	                if (dizziness < 1.25)
	                    ...
	                    with (objPlayer) {
	                        ...
	                        myyspeed = 0;
	                        legindex = 0;
	                        audio_play_sound(sndTinnitusHospital, 0, false);
	                        image_index = 0;
	                        image_speed = 0.15;
	                        sprite_index = sprPHospitalHeadache;
	                    }
	                }
	            }
