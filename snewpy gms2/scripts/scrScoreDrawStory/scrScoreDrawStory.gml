function scrScoreDrawStory() {
	...
	if finish = 1 draw_rectangle(camera_get_view_width(view_camera[0]) / 2 - string_width(string_hash_to_newline(finishstring)) * 0.375 - 3, camera_get_view_height(view_camera[0]) - 22, camera_get_view_width(view_camera[0]) / 2 + string_width(string_hash_to_newline(finishstring)) * 0.375 + 3, camera_get_view_height(view_camera[0]) + 3, 0)
	if update = 0 draw_set_font(fntScore)
	...
	if finish = 1 draw_text_transformed(camera_get_view_width(view_camera[0]) / 2 + lengthdir_x(0.5, dir), camera_get_view_height(view_camera[0]) - 11 + lengthdir_x(0.5, dir), string_hash_to_newline(finishstring), 0.75, 0.75, lengthdir_x(2, dir * 1.34 + 90))
	if ammoy > -32 draw_text_transformed(18 + string_width(string_hash_to_newline(ammostring)) * 0.5 + lengthdir_x(1, dir), camera_get_view_height(view_camera[0]) - ammoy + lengthdir_y(1, dir), string_hash_to_newline(ammostring), 1, 1, lengthdir_x(2, dir * 1.34))

	draw_set_color(color2)

	if ammoy > -32 draw_text_transformed(18 + string_width(string_hash_to_newline(ammostring)) * 0.5 + lengthdir_x(1, dir - 180), camera_get_view_height(view_camera[0]) - ammoy + lengthdir_y(1, dir - 180), string_hash_to_newline(ammostring), 1, 1, lengthdir_x(2, dir * 1.34 - 10))
	if finish = 1 draw_text_transformed(camera_get_view_width(view_camera[0]) / 2 + lengthdir_x(0.5, dir - 180), camera_get_view_height(view_camera[0]) - 11 + lengthdir_x(0.5, dir - 180), string_hash_to_newline(finishstring), 0.75, 0.75, lengthdir_x(2, dir * 1.34 + 100))
	if update = 0 draw_set_font(fntScore)
