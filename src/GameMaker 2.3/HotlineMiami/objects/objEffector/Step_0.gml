...
} else {if next=1 {if file_exists(working_directory+"\\restartmusic") file_delete(working_directory+"\\restartmusic") if newsong=1 {sxeasy_stop(1) sxeasy_setVolume(0) sxeasy_play(song);} room=nextroom} else {if room=rmSwatUpstairs and file_exists(working_directory+"\\restarmusic") {sxeasy_stop(1) bgm_Unload(global.currentsong)} scrResetSurfaces() game_load(working_directory+"\\tempsave.sav")}}}
}
